...
function starts() {
  if (
    JSON.parse(localStorage.getItem("member1")) == null ||
    JSON.parse(localStorage.getItem("member1")).length == 0 ||
    JSON.parse(localStorage.getItem("member2")) == null
  ) {
    Memberfetch();
    localStorage.setItem("member2", JSON.stringify(selectionList));
...
function renderList() {
  ...
  selectionList.push(detetedItem[0]);
  if (memberList.length == 0) {
    document.getElementById("counter").innerHTML = "Nothing to retrieve";
  } else {
    document.getElementById("counter").innerHTML = memberList.length;
